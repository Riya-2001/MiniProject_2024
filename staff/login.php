...
$password=md5($_POST['password']);
    ...
    if($query->rowCount() > 0)
        ...
        if ($role == 3) {
            $query = "SELECT ID,Email FROM tblstaff WHERE Email=:email AND Password=:password and Status = 'Active'";
            $query=$dbh->prepare($sql);
    $query->bindParam(':email',$email,PDO::PARAM_STR);
    $query-> bindParam(':password', $password, PDO::PARAM_STR);
    $query-> execute();
...
if(isset($_POST['login'])) 
  {
    $email=$_POST['email'];
    $password=md5($_POST['password']);
    $sql ="SELECT ID,Email FROM tblstaff WHERE Email=:email and Password=:password and Status = 'Active'";
    $query=$dbh->prepare($sql);
    $query->bindParam(':email',$email,PDO::PARAM_STR);
$query-> bindParam(':password', $password, PDO::PARAM_STR);
    $query-> execute();
